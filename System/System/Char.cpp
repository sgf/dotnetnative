...
namespace System
{
	...
	constexpr static bool IsLatin1(const uint16_t chr) noexcept { return static_cast<uint32_t>(chr) <= 0x00ff; }

	// Return true for all characters below or equal U+007f, which is ASCII.
	constexpr static bool IsAscii(const uint16_t chr) noexcept { return static_cast<uint32_t>(chr) <= 0x007f; }


	constexpr static bool IsInRange(const uint16_t c, const uint16_t min, const uint16_t max) noexcept { return (static_cast<uint32_t>(c) - static_cast<uint32_t>(min)) <= (static_cast<uint32_t>(max) - static_cast<uint32_t>(min)); }
	...
	static UnicodeCategory GetLatin1UnicodeCategory(const uint16_t ch)
	{
	...
	static constexpr bool IsWhiteSpaceLatin1(const uint16_t c)
	{
	...
	static constexpr bool IsSeparatorLatin1(const uint16_t c)
	{
		// U+00a0 = NO-BREAK SPACE
	...
	bool Char::IsDigit(const uint16_t c) noexcept
	{
		if (IsLatin1(c))
		{
	...
	bool Char::IsLetter(uint16_t c) noexcept
	{
		if (IsLatin1(c))
		{
			if (IsAscii(c))
	...
	bool Char::IsWhiteSpace(const uint16_t c) noexcept
	{
		if (IsLatin1(c))
		{
	...
	bool Char::IsUpper(const uint16_t c) noexcept
	{
		if (IsLatin1(c))
		{
			if (IsAscii(c))
	...
	bool Char::IsLower(const uint16_t c) noexcept
	{
		if (IsLatin1(c))
		{
			if (IsAscii(c))
	...
	bool Char::IsPunctuation(const uint16_t c) noexcept
	{
		if (IsLatin1(c))
		{
	...
	bool Char::IsLetterOrDigit(const uint16_t c) noexcept
	{
		if (IsLatin1(c))
		{
	...
	bool Char::IsControl(const uint16_t c) noexcept
	{
		if (IsLatin1(c))
		{
	...
	bool Char::IsNumber(const uint16_t c) noexcept
	{
		if (IsLatin1(c))
		{
			if (IsAscii(c))
	...
	bool Char::IsSeparator(const uint16_t c) noexcept
	{
		if (IsLatin1(c))
		{
	...
	bool Char::IsSurrogate(const uint16_t c) noexcept
	{
		return IsInRange(c, CharUnicodeInfo::HIGH_SURROGATE_START, CharUnicodeInfo::LOW_SURROGATE_END);
	}

	bool Char::IsSymbol(const uint16_t c) noexcept
	{
		if (IsLatin1(c))
		{
	...
	UnicodeCategory Char::GetUnicodeCategory(const uint16_t c) noexcept
	{
		if (IsLatin1(c))
		{
	...
	double Char::GetNumericValue(const uint16_t c) noexcept
	{
		return CharUnicodeInfo::GetNumericValue(c);
	}

	bool Char::IsHighSurrogate(const uint16_t c) noexcept
	{
		return IsInRange(c, CharUnicodeInfo::HIGH_SURROGATE_START, CharUnicodeInfo::HIGH_SURROGATE_END);
	}

	bool Char::IsLowSurrogate(const uint16_t c) noexcept
	{
		return IsInRange(c, CharUnicodeInfo::LOW_SURROGATE_START, CharUnicodeInfo::LOW_SURROGATE_END);
	}

	bool Char::IsSurrogatePair(const uint16_t highSurrogate, const uint16_t lowSurrogate) noexcept
	{
	...
	int Char::ConvertToUtf32(const uint16_t highSurrogate, const uint16_t lowSurrogate) noexcept
	{
