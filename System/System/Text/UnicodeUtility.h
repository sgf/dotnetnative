...
namespace System::Text {
	...
	public:
		...
		/// </summary>
		static constexpr void GetUtf16SurrogatesFromSupplementaryPlaneScalar(uint32_t value, uint16_t& highSurrogateCodePoint, uint16_t& lowSurrogateCodePoint)
		{
			assert(IsValidUnicodeScalar(value) && !IsBmpCodePoint(value));

			// This calculation comes from the Unicode specification, Table 3-5.

			highSurrogateCodePoint = static_cast<uint16_t>((value + ((0xD800u - 0x40u) << 10)) >> 10);
			lowSurrogateCodePoint = static_cast<uint16_t>((value & 0x3FFu) + 0xDC00u);
		}
	};
}
