...
namespace System
{
		...
		, m_capacity(str.Length())
	{
		if (str.Length() > 0)
		{
			...
			m_blocks->m_characters = DNN_make_unique_array(uint16_t[], str.Length());
			...
			const size_t copySize = sizeof(uint16_t) * str.Length();

			memcpy_s(m_blocks->m_characters.get(), copySize, str.m_string.get(), copySize);
		}
	}

	StringBuilder::StringBuilder(const uint16_t* str, const int length)
		: m_currentBlock(nullptr)
		, m_length(length)
		, m_capacity(length)
	{
		...
		if (length > 0)
		{
			m_blocks = DNN_make_unique(Block);
			m_blocks->m_nextBlock = nullptr;
			m_blocks->m_characters = DNN_make_unique_array(uint16_t[], length);
			...
			const size_t copySize = sizeof(uint16_t) * length;
		...
		, m_capacity(length)
	{
		...
		if (length > 0)
		{
			...
			{
				m_blocks->m_characters.get()[i] = static_cast<uint16_t>(str[i]);
			}
		}
	}
		...
		, m_capacity(capacity)
	{
		...
		if (capacity > 0)
		{
			m_blocks = DNN_make_unique(Block);
			m_blocks->m_nextBlock = nullptr;
			m_blocks->m_characters = DNN_make_unique_array(uint16_t[], capacity);
		...
		, m_capacity(copy.m_length)
	{
		if (copy.m_length > 0)
		{
			m_blocks = DNN_make_unique(Block);
			m_blocks->m_nextBlock = nullptr;
			m_blocks->m_characters = DNN_make_unique_array(uint16_t[], copy.m_length);
	...
	StringBuilder& StringBuilder::operator=(const uint16_t* str)
	{
		SetLength(0);
	...
	uint16_t StringBuilder::operator[](const int index) const&
	{
	...
	uint16_t& StringBuilder::operator[](const int index)&
	{
		if (index < 0 || index >= m_length)
		{
			throw IndexOutOfRangeException();
	...
	String StringBuilder::ToString(const int startIndex, const int length) const
	{
		...
		if (length > 0)
		{
			...
			const size_t destSize = sizeof(uint16_t) * length;
			str = shared_ptr<uint16_t[]>(DNN_New uint16_t[static_cast<size_t>(length) + 1], Deleter<uint16_t[]>(), DNN_Allocator(uint16_t[]));
			...
			else
			{
				...
				memcpy_s(str.get(), destSize, blockIter->m_characters.get(), sizeof(uint16_t) * numChars);
				...
				while (blockIter = blockIter->m_nextBlock.get())
				{
					if (numChars + blockIter->m_count >= length)
					{
						...
						memcpy_s(str.get() + numChars, destSize - (sizeof(uint16_t) * numChars), blockIter->m_characters.get() + blockIter->m_offset, remaining * sizeof(uint16_t));

						break;
					}

					memcpy_s(str.get() + numChars, destSize - (sizeof(uint16_t) * numChars), blockIter->m_characters.get() + blockIter->m_offset, blockIter->m_count * sizeof(uint16_t));
	...
	StringBuilder& StringBuilder::Remove(int startIndex, int length)
	{
		...
		while (blockIter && length > 0)
		{
			...
			else
			{
				...
				memmove_s(blockIter->m_characters.get() + blockIter->m_offset + blockOffset, sizeof(uint16_t) * blockIter->m_count, blockIter->m_characters.get() + blockIter->m_offset + endOffset, sizeof(uint16_t) * remaining);
	...
	StringBuilder& StringBuilder::Append(const uint16_t value)
	{
	...
	{
		return Append(static_cast<uint16_t>(value));
	}

	StringBuilder& StringBuilder::Append(const uint16_t value, int repeatCount)
	{
		if (repeatCount == 0)
		{
			return *this;
		...
		while (repeatCount > 0)
		{
			...
			uint16_t* destPtr = m_currentBlock->m_characters.get() + m_currentBlock->m_offset + m_currentBlock->m_count;
	...
	StringBuilder& StringBuilder::Append(const uint16_t* value, int length)
	{
		if (length == 0)
		{
			return *this;
		...
		while (length > 0)
		{
			...
			uint16_t* destPtr = m_currentBlock->m_characters.get() + m_currentBlock->m_offset + m_currentBlock->m_count;
			const size_t size = sizeof(uint16_t) * iterCount;
	...
	StringBuilder& StringBuilder::Append(const char* value, int length)
	{
		...
		while (length > 0)
		{
			...
			for (int i = 0; i < iterCount; ++i)
			{
				destPtr[i] = static_cast<uint16_t>(value[i]);
			}
	...
	StringBuilder& StringBuilder::Append(const StringBuilder& value)
	{
		...
		do
		{
			if (srcCount > 0)
			{
				...
				{
					memcpy_s(m_currentBlock->m_characters.get() + destBlockOffset, sizeof(uint16_t) * available, srcPtr->m_characters.get() + srcBlockOffset, sizeof(uint16_t) * srcCount);
				...
				{
					const size_t size = sizeof(uint16_t) * available;
	...
	void StringBuilder::CopyBlocks(uint16_t* destination, const int destSize, const StringBuilder& src)
	{
		int destOffset = 0;
		...
		for (Block* blockIter = src.m_blocks.get(); blockIter; blockIter = blockIter->m_nextBlock.get())
		{
			...
			memcpy_s(destination + destOffset, sizeof(uint16_t) * (destSize - destOffset), blockIter->m_characters.get() + blockIter->m_offset, sizeof(uint16_t) * blockIter->m_count);
	...
	unique_ptr<StringBuilder::Block> StringBuilder::AllocateBlock(const int capacity)
	{
		...
		block->m_characters = DNN_make_unique_array(uint16_t[], capacity);
