...
namespace System
{
	CharEnumerator::CharEnumerator(const shared_ptr<uint16_t[]>& chars, const int length)
		: m_chars(chars)
		, m_length(length)
		, m_index(-1)
		, m_current(0)
	...
	const uint16_t& CharEnumerator::Current() const&
	{
		if (m_index < 0)
		{
	...
	// Returns:
	//     The element in the collection at the current position of the enumerator.
	uint16_t& CharEnumerator::Current()&
	{
		if (m_index < 0)
		{
			throw new InvalidOperationException("Enumeration not started.");
