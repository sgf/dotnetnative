...
namespace System
{
	...
	static uint8_t InternalGetCategoryValue(const int ch, const int offset)
	{
		...
		{
			index = System::Buffers::Binary::BinaryPrimitives::ReverseEndianness(static_cast<uint16_t>(index));
		}
	...
	static double InternalGetNumericValue(const int ch)
	{
		...
		if (static_cast<uint32_t>(index) < static_cast<uint32_t>(std::size(NumericLevel1Index)))
		{
			...
			return BitConverter::Int64BitsToDouble(System::Buffers::Binary::BinaryPrimitives::ReverseEndianness(*reinterpret_cast<const int64_t*>(value)));
		}

		return -1;
	}

	UnicodeCategory CharUnicodeInfo::GetUnicodeCategory(const uint16_t ch)
	{
	...
	double CharUnicodeInfo::GetNumericValue(const uint16_t ch)
	{
